...
func populate_item_grid() -> void:
	...
	for i in inventory_data.slot_datas.size():
		...
		var light_data: LightSlotData = shelf_inventory_data.light_slot_datas[i]
		...
		slot.set_light_level(light_data)

func update_slot_light(index: int) -> void:
	var shelf_inventory_data := inventory_data as ShelfInventoryData
	var light_data: LightSlotData = shelf_inventory_data.light_slot_datas[index]
